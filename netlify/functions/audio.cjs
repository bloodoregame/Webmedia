...
app.use((req, res, next) => {
  console.log(`[AUDIO] ${req.method} ${req.path}`);
  next();
});
...
app.get('/:filename', async (req, res) => {
  try {
    ...
    if (!trackInfo) {
      // Vérifier si c'est un fichier généré dynamiquement
      if (filename.startsWith('track-')) {
        console.log('Serving dynamically generated track');
        ...
        // Génère des données audio aléatoires (pour la démo)
...
app.get('/:filename/stream', async (req, res) => {
  try {
    const filename = req.params.filename;
    console.log(`Requested streaming for: ${filename}`);
    
    let trackInfo = mockAudioFiles[filename];
    
    // Vérifier si c'est un fichier généré dynamiquement
    if (!trackInfo && filename.startsWith('track-')) {
      trackInfo = {
        title: 'Track téléchargé',
        artist: 'Utilisateur'
      };
    }
